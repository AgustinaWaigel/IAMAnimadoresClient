          ...
          <div key={cat}>
            ...
            <div className="grid gap-4 md:grid-cols-2 items-start">
              {gruposFiltrados.map(([grupoId, recursos]) => {
                const objetivoComun = (recursos as any[])[0]?.objetivo || "Sin objetivo";
                return (
        ...
        <>
          ...
          {mostrarFormulario && (
            <form onSubmit={handleUpload} className="bg-gray-100 p-4 rounded shadow space-y-4 mt-4">
              ...
              <input key={fileKey} type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.gif,.webp" multiple onChange={(e) => setArchivo(Array.from(e.target.files || []))} className="w-full" />
